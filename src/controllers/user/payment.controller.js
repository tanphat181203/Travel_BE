...
export const createPayment = async (req, res) => {
  try {
    ...
    const successfulCheckout = existingCheckouts.find(checkout => checkout.payment_status === 'completed');
    ...
    logger.info(`Created payment for booking ${booking_id}, checkout ID: ${checkout.checkout_id}`);
...
export const vnpayReturn = async (req, res) => {
  try {
    ...
    if (!verify.isVerified) {
      logger.warn(`VNPay return verification failed: ${JSON.stringify(req.query)}`);
      return res.redirect(`${process.env.CLIENT_URL}/payment/failed?reason=verification`);
    }
    
    if (!verify.isSuccess) {
      logger.warn(`VNPay payment failed: ${JSON.stringify(req.query)}`);
      return res.redirect(`${process.env.CLIENT_URL}/payment/failed?reason=payment`);
    }
    ...
    if (!checkout) {
      ...
      return res.redirect(`${process.env.CLIENT_URL}/payment/failed?reason=not_found`);
    }
    ...
    logger.info(`Payment successful for booking ${checkout.booking_id}, transaction: ${txnRef}`);
    
    return res.redirect(`${process.env.CLIENT_URL}/payment/success?booking_id=${checkout.booking_id}`);
  } catch (error) {
...
export const vnpayIPN = async (req, res) => {
  try {
    ...
    if (!verify.isVerified) {
      logger.warn(`VNPay IPN verification failed: ${JSON.stringify(req.query)}`);
    ...
    if (parseFloat(checkout.amount) !== parseFloat(amount)) {
      logger.error(`Amount mismatch for transaction ${txnRef}: expected ${checkout.amount}, got ${amount}`);
      return res.status(200).json(getErrorResponse());
    }
    ...
    logger.info(`IPN confirmed payment for booking ${checkout.booking_id}, transaction: ${txnRef}`);
